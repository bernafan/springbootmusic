...
@Controller
@RequestMapping("playlist/{playlistId}/musicas")    // recebe requisao em localhost:8080/playlists
public class MusicController {
    ...
    public ModelAndView listar(@PathVariable("playlistId")
        ...
        model.addAttribute("musicas",
                musicaService.recuperarPorPlaylist(playlistId));
        model.addAttribute("playlistId", playlistId);
        return new ModelAndView("/musicas/list", model);
    }

    @GetMapping("/cadastro")
    public String preSalvar(@ModelAttribute("musica") Musica musica,
                            @PathVariable("playlistId") long playlistId) {
        return "/musicas/add";
    }

    @PostMapping("/salvar")
    public String salvar(@PathVariable("playlistId")
                                 long playlistId, @Valid @ModelAttribute("musica")
    ...
    public ModelAndView preAtualizar(@PathVariable("playlistId")
                                             long playlistId,
                                     @PathVariable("musicaId") long musicaId, ModelMap model) {
        Musica musica = musicaService.recuperarPorPlaylistIdEMusicaId
                (playlistId, musicaId);
        model.addAttribute("musica", musica);
        model.addAttribute("playlistId", playlistId);
        return new ModelAndView("/musicas/add", model);
    }

    @PutMapping("/salvar")
    public String atualizar(@PathVariable("playlistId") long playlistId,
    ...
    public String remover(@PathVariable("playlistId")
                                  long playlistId, @PathVariable("musicaId")
